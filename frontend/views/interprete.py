...
def render() -> None:
    ...
    .interprete-badge { display: inline-block; background: linear-gradient(135deg, #7B5EA7, #C084FC); color: white; font-family: 'Raleway', sans-serif; font-size: 0.7rem; font-weight: 600; letter-spacing: 0.2em; text-transform: uppercase; padding: 0.25rem 0.9rem; border-radius: 20px; margin-bottom: 0.75rem; }
    .interprete-title { font-family: 'Cinzel', serif; font-size: 1.8rem; font-weight: 900; background: linear-gradient(135deg, #7B5EA7, #C084FC, #F0D0FF); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; margin-bottom: 0.3rem; }
    .interprete-sub { font-family: 'Raleway', sans-serif; font-size: 0.88rem; color: #8A7AA0; margin-bottom: 1.5rem; }
    .rctc-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 0.5rem; margin: 0.5rem 0 1.5rem; }
    .rctc-card { background: linear-gradient(160deg, #1A0F2E, #12122B); border: 1px solid #7B5EA733; border-radius: 10px; padding: 0.75rem; text-align: center; }
    .rctc-letter { font-family: 'Cinzel', serif; font-size: 1.4rem; font-weight: 900; color: #C084FC; display: block; }
    .rctc-word { font-family: 'Raleway', sans-serif; font-size: 0.68rem; color: #8A7AA0; letter-spacing: 0.1em; text-transform: uppercase; }
    .metric-box { background: #0F172A; border: 1px solid #334155; border-radius: 10px; padding: 0.8rem 1.2rem; display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; font-family: 'Plus Jakarta Sans', sans-serif; font-size: 0.85rem; color: #94A3B8; }
    .metric-box .valor { font-size: 1.05rem; font-weight: 700; color: #4ADE80; }
    .metric-box .etiqueta { font-size: 0.73rem; color: #64748B; margin-top: 2px; }
    .problema-box { background: linear-gradient(160deg, #1A0F2E, #12122B); border: 1px solid #7B5EA733; border-radius: 12px; padding: 1.2rem 1.4rem; font-family: 'Plus Jakarta Sans', sans-serif; font-size: 0.88rem; color: #CBD5E1; line-height: 1.6; height: 100%; }
    .problema-title { font-family: 'Raleway', sans-serif; font-size: 0.72rem; font-weight: 600; letter-spacing: 0.15em; text-transform: uppercase; color: #C084FC; margin-bottom: 0.6rem; }
    </style>
    ...
    with col1:
        ...
        <div class="problema-box">
            ...
            <br><br>
            <div class="problema-title">La Solución: Arquitectura Filtro</div>
    ...
    if boton_ejecutar:
        ...
        if not prompt_usuario.strip():
            ...
            if not api_key:
                st.error("GEMINI_API_KEY no configurada en Secrets. Ve a Configuración.")
            else:
                with st.spinner("Reestructurando sintaxis bajo framework RCTC..."):
                    try:
                        ...
                        tipo_contexto = {
                            ...
                            "LinkedIn":       "redacción de posts profesionales en LinkedIn",
...
- ROLE: Define el rol exacto que debe asumir el modelo.
- CONTEXT: Proporciona el contexto mínimo necesario.
- TASK: Describe la tarea de forma inequívoca.
- CONSTRAINT: Establece restricciones de tono, estilo, longitud.

Responde ÚNICAMENTE con estos dos bloques:

[PROMPT_FINAL]
El prompt estructurado siguiendo RCTC.

[CONSEJOS]
Lista de 3 a 5 puntos concretos explicando qué tenía el prompt original que generaba ineficiencia."""

                        response = client.models.generate_content(
                            model='gemini-2.5-flash',
                            contents=(f"Prompt original:\n\n{prompt_usuario}\n\nOptimízalo para {tipo_contexto} siguiendo RCTC."),
                        ...
                        if not parse_ok:
                            st.warning("El modelo no devolvió el formato esperado. Se muestra la respuesta completa.")
                        ...
                        with tab1:
                            ...
                            with c2:
                                if st.button("Video AI", use_container_width=True):
                                    ...
                                    st.rerun()
                            with c3:
                                if st.button("Redactor LinkedIn", use_container_width=True):
                                    st.session_state["linkedin_topic_inject"] = prompt_final
                                    st.session_state["vista"] = "Redactor LinkedIn"
                                    st.rerun()
                        ...
                        with tab2:
                            if consejos_final:
                                ...
                                st.info("No se pudo extraer la auditoría.")
